...
\noindent Trong một ứng dụng thông thường ta chỉ sử dụng một database cho cả việc đọc và ghi, nếu ứng dụng có lượt truy cập không cao thì sử dụng một database cũng đủ đáp ứng yêu cầu cho người dùng. Nhưng đối với các ứng dụng mà có lượt truy cập cao thì chỉ sử dụng một database cho cả việc đọc và ghi sẽ dẫn tới database bị quá tải và ứng dụng không thể xử lý nổi tất cả các yêu cầu của người dùng.
...
spec:
  ...
  ports:
    - port: 5432
\end{lstlisting}
\noindent Để truyền hai tệp tin cấu hình trên vào database, ta sẽ sử dụng ConfigMaps dạng volumeMounts. Ta tiến hành tạo một thư mục tên là config và tạo thêm hai tệp tin tên là postgresql.conf và pg\_hba.conf bên trong thư mục đó.\\
...
\end{lstlisting}
\textbf{3. Chuẩn bị dữ liệu cho slave database}\\
...
\end{lstlisting}
\noindent Như vậy, với kết quả trên, ta có thể thấy jobs đồng bộ dữ liệu đã được triển khai thành công. \\
\textbf{4. Tạo slave database}\\
\noindent Ta sẽ tiến hành tạo Slave DB. Đầu tiên ta sẽ tạo ConfigMaps để chứa cấu hình Slave DB, tạo thư mục tên là slave-config với hai tệp tin là postgresql.conf với recovery.conf.
